...
def send(msg):
    url = f"https://api.telegram.org/bot8902701930:AAGvfU8HwnRIY6KFABZqy6bOc8qH2zaWx8M/sendMessage"
    requests.post(url, data={"chat_id": 436233523, "text": msg})
...
def binance_listings():
    ...
    for x in items:
        title = x.get("title", "")
        if "List" in title or "listing" in title.lower():
...
def binance_delistings():
    ...
    for x in items:
        title = x.get("title", "")
        if "Delist" in title or "delist" in title.lower():
            results.append("❌ Binance Delisting:\n" + title)
    return results

# -----------------------------
# 3) CoinMarketCap Announcements
# -----------------------------
